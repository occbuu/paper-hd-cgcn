...
Không làm Mục 7 (cắt từ) cho đến khi Mục 1–6 đã vào bài.

Hình dùng: `outputs/replication/fig_mds.png`, `fig_k_scan.png`.
...
**Caption:** Figure 3. MDS of the 34 documents in TF-IDF space, coloured by NMF topic (K = 7).

**File hình:** `outputs/replication/fig_mds.png`
...
**Caption bảng:** Table A1. Topic-number scan on the 34-document corpus. Silhouette uses cosine distance. K = 7 is an interpretive choice, not the silhouette maximum.

**Hình phụ (tuỳ chọn, Appendix):** `outputs/replication/fig_k_scan.png`  
...
Two robustness checks were completed on the same specification.
...
**Giữ nguyên** các câu sau đó về replication texts, contracts/judgments, và cut-off 16 September 2026.

**Xong khi:** Kết luận khớp với Lần 4 (đã chạy test, không còn “sẽ kiểm tra”).
...
The replication package accompanying this article contains: (i) a document inventory of the thirty-four files, with identifiers, filenames and a genre flag; (ii) the preprocessing and topic-modelling specification, including the Vietnamese lexicon, stop-word list, domain multi-word terms, TF-IDF settings (minimum document frequency 2, maximum document frequency 0.9), NMF initialisation (nndsvda) and random seed 42; (iii) document–topic assignments at K = 7; (iv) topic-keyword tables; and (v) the files needed to redraw Figure 3 and Table A1. The package does not include copyrighted full texts of the underlying news or journal articles; those remain in the authors’ research files and can be inspected for verification subject to the original publishers’ terms.

**Gói file thật:** thư mục `outputs/replication/` (và script trong `hopdong-CGCN/src/Dinh_Tinh/`). Khi nộp, zip thư mục này thành supplementary material.

**Xong khi:** có mục Data availability và file zip đi kèm.
...
**Không cắt:** vai kép Bùi (2019); leave-one-out; news-only; NĐ 101/2026; Luật AI; Chương bán dẫn Luật 71/2025.
